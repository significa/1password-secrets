...
def create_1password_secrets(
    ...
    return json.loads(
        subprocess.check_output([
            ...
            get_last_edited_1password_custom_field_cli_parameter(),
            '--format',
            'json',
        ])
    )
...
def update_1password_custom_field(item_id, field, value):
    ...
    subprocess.check_output([
        ...
        'edit',
        item_id,
        get_1password_custom_field_cli_parameter(field, value),
        '--format',
        'json'
    ])


def get_1password_custom_field_cli_parameter(field_name, value):
    PREFIX = 'Generated by 1password-secrets'
    return f'{PREFIX}.{field_name}[text]={value}'


def get_last_edited_1password_custom_field_cli_parameter():
    now_formatted = datetime.now().strftime(DATE_FORMAT)

    return get_1password_custom_field_cli_parameter(
